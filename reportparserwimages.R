...
curvecolor <- function(ploc, tdf, df) {
...
for (i in 1:imax)
{
        ...
        if (length(g_lci) == 0 | length(g_mzi) ==0)  { 
                ...
                if (length(g_lci) > 1) {
                        ...
                        lc_shd <- curvecolor(lc_peak, t_lci_LC, lc_data)
                        ...
                        flr_shd <- curvecolor(flr_peak, t_lci_flr, flr_data)
                        
                        #Find Range Values
                range <- type.convert(txti[grep("MaxIntensity", txti[,1]),2])
                lc_dis_range <- paste("Range:", round(range[1]/1000000, digits=3))
                flr_dis_range <- paste("Range:", range[2])
                ...
                lc_shd <- curvecolor(peakloc, t_lci, txti)
                #Find LC range value
                range <- type.convert(txti[grep("MaxIntensity", txti[,1]),2])
                lc_dis_range <- paste("Range:", round(range/1000000, digits=3))
                }
                ...
                lclabs  <- c(paste("HPLC:",dfb),"",lcl_1,lcl_2,lcl_3,"Time","Absorbance %","o", lc_dis_range)
                
                fn_mz <- coa_plot(x_mzi,y_mzi,x_txt,y_txt,x_txt,TRUE,FALSE,dfb,"mz",mzlabs, l_col="red", so=SO[i])        # mass spec
                
                if (length(g_lci) == 1) {
                fn_lc <- coa_plot(x_lci,y_lci,x_peak,y_peak,y_peak_txt,FALSE,TRUE,dfb,"lc",lclabs, shd=lc_shd, so=SO[i])	# hplc
                } else {
                        fn_lc <- coa_plot(x_lci_LC, y_lci_LC, x_peak_lc, y_peak_lc, y_peak_txt_lc,FALSE, TRUE, dfb, "lc", lclabs, shd=lc_shd, so=SO[i])
