library(shiny)
imgpath <- "//DATA4/Mass Spec DataRrepository"
        ...
        contentInput <- reactive({ 
                ...
                isolate({
                        ...
                        for (i in 1:length(SO_list)){
                                SO <- SO_list[i]
                                img_loc <- paste(imgpath, "imgs", SO, sep="/")
                                ship_loc <- paste(imgpath, "Shipping", SO, sep="/")
                                ...
                                printed_loc <- paste(imgpath,"/imgs/", "printed_", SO, sep="")
