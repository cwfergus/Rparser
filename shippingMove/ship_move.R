path <- "//DATA4/Mass Spec DataRrepository"




read.table("test.tab", sep="\t", col.names = c("SO", "SS"), stringsAsFactors=FALSE) -> data
...
for (i in 1:length(SO_list)){
        SO <- SO_list[i]
        img_loc <- paste(path, "imgs", SO, sep="/")
        ship_loc <- paste(path, "Shipping", SO, sep="/")
        ...
        printed_loc <- paste(path,"/imgs/", "printed_", SO, sep="")
        file.rename(img_loc, printed_loc)
        message <- append(message, temp_msg)
}
...
SO_list <- unique(data$SO)
message <- data.frame(SOchr="SO", suc_move="Images Processed?", stringsAsFactors=FALSE)
for (i in 1:length(SO_list)){
        SO <- SO_list[i]
        ...
        row <- cbind(SOchr, suc_move)
        message <- rbind(message, row)
}
message
