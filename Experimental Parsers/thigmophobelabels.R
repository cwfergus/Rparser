...
coa_plot <- function(x,y,x_txt,y_txt,l_txt,pt_b,pt_c,fb,gr_t,labels,l_color="black")
{
        ...
        plot(x, y, 
             ...
             col.lab="blue", col.axis="blue", ylim = c(0, 105)
        )
        if (pt_b) {
                if (length(l_txt)>=10) {
                        min <- 0.5*max(y_txt)
...
for (i in 1:imax)
{
        ...
        if (length(g_lci) == 0 | length(g_mzi) ==0)  { 
                ...
                fn_mz <- coa_plot(x_mzi,y_mzi,x_txt,y_txt,l_txt,TRUE,FALSE,dfb,"mz",mzlabs, l_col="red")        # mass spec
                fn_lc <- coa_plot(x_lci,y_lci,x_lci_min,y_lci_min,y_lci_min_txt,FALSE,TRUE,dfb,"lc",lclabs)	# hplc
