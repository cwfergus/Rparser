...
if (file.exists("Y:/oldreports.txt") == FALSE) {
        filestoreadin <- filenames
} else {
        scan("Y:/oldreports.txt", what = character(), sep="\t")-> oldreports
        filestoreadin <- filenames[!filenames %in% oldreports]
}

if (length(filestoreadin) == 0){
        stop("No new report files to read in", call. = FALSE)
}
...
write(filestoreadin, file="Y:/oldreports.txt", sep = "\t", append=TRUE)
