...
coa_plot <- function(x,y,x_txt,y_txt,l_txt,pt_b,pt_c,outfile,labels,l_color="black", shd=NULL, so){
        ...
        png(filename=outfile, width=680, height=440)        # Open a device: bmp & pdf are other options here
...
shinyServer(function(input, output, session){
        ...
        observe({
                ...
                for (o in 1:(imax)) {
                        ...
                        mzfile <- tempfile(fileext='.png')
                        lcfile <- tempfile(fileext='.png')
                        mzoutfile <- append(mzoutfile, mzfile)
                        lcoutfile <- append(lcoutfile, lcfile)
                        allfiles <- append(allfiles, c(mzfile, lcfile))
                ...
                for (i in 1:imax)
                {
                        ...
                        if (length(g_lci) == 0 | length(g_mzi) ==0)  { 
                                ...
                                coa_plot(x_mzi,y_mzi,x_txt,y_txt,x_txt,TRUE,FALSE,mzoutfile[i],mzlabs, l_col="red", so=SO[i])        # mass spec
                                        if (length(g_lci) == 1) {
                                                coa_plot(x_lci,y_lci_au,x_peak,lc_range,y_peak_txt,FALSE,TRUE,lcoutfile[i],lclabs, shd=lc_shd, so=SO[i])	# hplc
                                        } else {
                                                coa_plot(x_lci_LC, y_lci_LC_au, x_peak_lc, lc_range, y_peak_txt_lc,FALSE, TRUE, lcoutfile[i], lclabs, shd=lc_shd, so=SO[i])
                                                ...
                                                coa_plot(x_lci_flr, y_lci_flr_au, x_peak_flr, flr_range, y_peak_txt_flr,FALSE,TRUE,floutfile[i],flrlabs, shd=flr_shd, so=SO[i])
                                        }
                ...
                isolate({
                        plotnames4 <- str_extract(allfiles, "file[[:alnum:]]+[[:punct:]]png")
                        output$genplots <- renderUI({
                                plot_output_list <- lapply(1:length(allfiles), function(i){
                                        plotname <- plotnames4[i]
                                        plotOutput(plotname)
                                })
                                do.call(tagList, plot_output_list)
                        })
                        for (t in 1:length(allfiles)) {
                                local({
                                        my_i <- t
                                        plotname <- plotnames4[my_i]
                                        output[[plotname]] <- renderImage({
                                                myname <- allfiles[my_i]
                                                list(src = myname,
                                                     contentType = 'image/png',
                                                     width = 680,
                                                     height = 440)
        ...
        observe({
                if(is.null(input$QCBrefresh) || input$QCBrefresh==0) return(NULL)
                QCB_options2 <- list.files(data_path_rpt, pattern = "*.rpt", full.names = FALSE )
                updateSelectizeInput(session, "QCB", "Input QC Barcode", 
                                     choices = c(Refreshed="", QCB_options2))
